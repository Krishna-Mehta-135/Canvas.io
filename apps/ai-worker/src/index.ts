...
function extractCurrentCanvasShapes(prompt: string): Array<Record<string, unknown>> {
    ...
    try {
        ...
        return parsed.filter((s): s is Record<string, unknown> => typeof s === "object" && s !== null);
    } catch {
        return [];
    }
}

function translateShapes(shapes: Array<Record<string, unknown>>, dx: number, dy: number): Array<Record<string, unknown>> {
...
function collidesWithAny(candidate: Bounds, existing: Bounds[], padding: number): boolean {
...
function placeGeneratedShapesInEmptyRegion(
    ...
    if (!collidesWithAny(preferredCandidate, existingShapeBounds, COLLISION_PADDING)) {
    ...
    const strideX = Math.max(220, (generatedBounds.maxX - generatedBounds.minX) + GAP_X);
    const strideY = Math.max(160, (generatedBounds.maxY - generatedBounds.minY) + GAP_Y);
...
function extractJsonArrayString(rawText: string): string {
    ...
    if (startIdx === -1) {
        throw new Error(`Gemini response does not contain a JSON array start token '[': ${rawText.slice(0, 240)}`);
    }
    if (endIdx === -1 || endIdx < startIdx) {
        throw new Error(`Gemini response appears truncated (missing closing ']'): ${rawText.slice(0, 240)}`);
    }
...
function validateAndNormalizeShapes(parsed: unknown): unknown[] {
    if (!Array.isArray(parsed)) {
        throw new Error(`Gemini response was not a JSON array. Got ${typeof parsed}: ${JSON.stringify(parsed).slice(0, 240)}`);
    }
    ...
    const shapesWithIds = (parsed as unknown[]).map((shape) => {
        ...
        const needsId = !existingId || existingId.length < 8 || usedIds.has(existingId);
...
function buildAttemptPrompt(basePrompt: string, attempt: number, previousIssue?: string): string {
    if (attempt === 0) {
        return basePrompt;
    }
...
async function generateShapesFromPrompt(prompt: string): Promise<unknown[]> {
    ...
    throw new Error(`AI generation failed after ${MAX_GENERATION_ATTEMPTS} attempts: ${lastError}`);
}

// ---------------------------------------------------------------------------
// Post result back to HTTP backend
// ---------------------------------------------------------------------------
async function postResult(jobId: string, shapes?: unknown[], errorMessage?: string) {
    await axios.post(
...
async function handleAiJob(job: AiGenerateJob): Promise<void> {
    console.log(`[AI Worker] Processing job ${job.jobId} — prompt: "${job.prompt.slice(0, 80)}..."`);

    try {
        const shapes = await generateShapesFromPrompt(job.prompt);
        console.log(`[AI Worker] Job ${job.jobId} done — generated ${shapes.length} shapes`);
        ...
        try {
            ...
            console.error(`[AI Worker] Failed to report error for job ${job.jobId}:`, postErr);
        }
    }
}
