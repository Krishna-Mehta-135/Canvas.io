...
vi.mock("jsonwebtoken", () => ({
  default: {
    ...
    verify: vi
      .fn()
      .mockReturnValue({ userId: "user-1", type: "password-reset" }),
  },
}));
...
describe("Auth Controller", () => {
  ...
  describe("signup", () => {
    it("should create a new user and set cookies", async () => {
      ...
      vi.mocked(tokenUtils.generateRefreshToken).mockReturnValue(
        "refresh-token",
      );
  ...
  describe("refreshAccessToken", () => {
    it("should refresh access token with valid refresh token", async () => {
      ...
      vi.mocked(tokenUtils.generateAccessToken).mockReturnValue(
        "new-access-token",
      );
  ...
  describe("resetPassword", () => {
    it("should reset password with valid token", async () => {
      req = {
        body: { token: "valid-reset-token", password: "new-password-123" },
      };
