...
describe('Room Controller - replaceShapes', () => {
    ...
    it('should replace shapes successfully', async () => {
        ...
        vi.mocked(prismaClient.room.findFirst).mockResolvedValue({ id: 1, adminId: 'user-123' } as any);
        vi.mocked(prismaClient.$transaction).mockImplementation(async (cb: any) => await cb(prismaClient));

        await replaceShapes(req as Request, res as Response, next);

        expect(prismaClient.shape.deleteMany).toHaveBeenCalledWith({ where: { roomId: 1 } });
...
describe('Room Controller - requestRoomAccess', () => {
    ...
    it('should create access request', async () => {
        ...
        vi.mocked(prismaClient.room.findFirst).mockResolvedValue({ id: 1, adminId: 'owner-id' } as any);
        vi.mocked(prismaClient.roomMember.findUnique).mockResolvedValue(null);
        vi.mocked(prismaClient.roomAccessRequest.findUnique).mockResolvedValue(null);
        vi.mocked(prismaClient.roomAccessRequest.create).mockResolvedValue({ id: 10, status: 'pending' } as any);
...
describe('Room Controller - AI functions', () => {
    ...
    it('should handle AI queue error', async () => {
        ...
        vi.mocked(publishAiGenerateJob).mockRejectedValueOnce(new Error('Queue down'));
    ...
    it('should get AI job status', async () => {
        ...
        await generateAiCanvas(req as Request, res as Response, next);
        const jobId = vi.mocked(res.json).mock.calls[0]?.[0].data.jobId;
...
describe('Room Controller - getRoomChatBootstrap', () => {
    ...
    it('should fetch chat bootstrap data with messages', async () => {
        ...
        vi.mocked(prismaClient.room.findUnique).mockResolvedValue({
            ...
            members: [
                { user: { id: 'member-1', name: 'Member', handle: 'member', photo: null } }
            ]
        } as any);
