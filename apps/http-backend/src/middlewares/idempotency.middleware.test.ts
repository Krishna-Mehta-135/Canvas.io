...
describe('idempotencyMiddleware', () => {
    ...
    it('should cache and replay response for same key and payload', async () => {
        vi.mocked(req.header).mockReturnValue('key-1');
        
        // First call
        let firstCallNext = vi.fn(() => {
    ...
    it('should return 409 if same key is used with different payload', async () => {
        ...
        await expect(idempotencyMiddleware(req2 as Request, res2 as Response, vi.fn()))
            .rejects.toThrow(ApiError);
    });

    it('should handle non-replayable status (500)', async () => {
        vi.mocked(req.header).mockReturnValue('key-error');
    ...
    it('should handle res.send body kind', async () => {
        vi.mocked(req.header).mockReturnValue('key-send');
    ...
    it('should handle res.end body kind', async () => {
        vi.mocked(req.header).mockReturnValue('key-end');
    ...
    it('should handle client closed before response (499)', async () => {
        vi.mocked(req.header).mockReturnValue('key-close');
