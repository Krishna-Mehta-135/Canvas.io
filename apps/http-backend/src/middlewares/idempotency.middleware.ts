...
const IDEMPOTENCY_WINDOW_MS = Number(
  process.env.IDEMPOTENCY_WINDOW_MS ?? "600000",
);
const IDEMPOTENCY_MAX_ENTRIES = Number(
  process.env.IDEMPOTENCY_MAX_ENTRIES ?? "2000",
);
...
export const idempotencyMiddleware: RequestHandler = async (req, res, next) => {
  ...
  const originalJson = res.json.bind(res) as unknown as (
    body: unknown,
  ) => unknown;
  const originalSend = res.send.bind(res) as unknown as (
    body?: unknown,
  ) => unknown;
  const originalEnd = res.end.bind(res) as unknown as (
    ...args: unknown[]
  ) => unknown;
  ...
  const finalize = () => {
    ...
    if (!isReplayableStatus(statusCode)) {
      ...
      rejectPending(
        new ApiError(statusCode, "Non-replayable idempotency response"),
      );
      return;
    }
  ...
  res.once("close", () => {
    ...
    if (!res.writableEnded) {
      failPending(
        new ApiError(499, "Request closed before a response could be cached"),
      );
    }
  });
