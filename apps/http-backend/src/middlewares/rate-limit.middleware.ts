...
export async function rateLimitMiddleware(req: Request, res: Response, next: NextFunction) {
	...
	try {
		...
		const limitResult = await checkRedisRateLimit(routeKey, MAX_REQUESTS, WINDOW_MS);
		res.setHeader("X-RateLimit-Limit", String(limitResult.limit));
		res.setHeader("X-RateLimit-Remaining", String(limitResult.remaining));
		res.setHeader("X-RateLimit-Reset", String(Math.ceil(limitResult.resetAtMs / 1000)));
		...
		console.warn("[rate-limit] Redis rate limit check failed; allowing request", {
			routeKey,
