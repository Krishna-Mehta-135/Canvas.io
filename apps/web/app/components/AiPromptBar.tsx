...
function mergeRegions(regions: Region[], gap = 40): Region[] {
    ...
    const sorted = [...regions].sort((a, b) => a.minX - b.minX || a.minY - b.minY);
    const merged: Region[] = [];
...
export function AiChatModal({
    ...
    useEffect(() => { if (isOpen) setTimeout(() => textareaRef.current?.focus(), 80); }, [isOpen]);
    ...
    const poll = (jobId: string, aiId: string, attempt = 0) => {
        ...
        if (attempt > POLL_MAX_ATTEMPTS) {
            patchMsg(aiId, {isGenerating: false, isError: true, text: "Timed out — please try again."});
            setIsGenerating(false);
            return;
        }
        pollTimerRef.current = setTimeout(async () => {
            try {
                const res = await apiClient.get(`${httpBackend}/room/${roomId}/ai/generate/${jobId}`);
                const d = (res.data as {data?: {status?: string; shapes?: unknown[]; errorMessage?: string}})?.data;
                if (d?.status === "done") {
                    const shapes = d.shapes ?? [];
                    patchMsg(aiId, {isGenerating: false, shapeCount: shapes.length, text: `Done! Added ${shapes.length} shapes to your canvas.`});
                ...
                patchMsg(aiId, {isGenerating: false, isError: true, text: "Network error — please try again."});
                setIsGenerating(false);
            }
        }, POLL_INTERVAL_MS);
    };
    ...
    const handleSend = async () => {
        ...
        addMsg({id: aid, role: "ai", text: "Generating…", isGenerating: true, time: nowTime()});
        setIsGenerating(true);

        try {
            const res = await apiClient.post(`${httpBackend}/room/${roomId}/ai/generate`, {prompt: finalPrompt});
            ...
            const backendMessage = maybeAxiosError.response?.data?.message || maybeAxiosError.message || "Could not start AI generation.";
            patchMsg(aid, {isGenerating: false, isError: true, text: backendMessage});
            setIsGenerating(false);
    ...
    const inputBg = isDark ? "bg-white/6 border-white/10 focus-within:border-indigo-400/40 focus-within:ring-indigo-500/10" : "bg-slate-50 border-slate-200 focus-within:border-indigo-400 focus-within:ring-indigo-100";
    const inputText = isDark ? "text-white placeholder-white/30" : "text-slate-900 placeholder-slate-400";
    const closeBtnCls = isDark ? "text-white/40 hover:bg-white/10 hover:text-white" : "text-slate-400 hover:bg-slate-100 hover:text-slate-700";
    ...
    return (
        ...
        >
            ...
            <div className={`flex shrink-0 items-center gap-3 rounded-t-2xl px-4 py-3 ${headerBg} border-b ${border}`}>
                {/* AI avatar */}
                <div
                ...
                <div className="flex-1 min-w-0">
                    <p className={`text-sm font-semibold leading-tight truncate ${headerTitle}`}>Canvas AI</p>
                    <p className={`text-[10px] leading-tight ${isDark ? "text-indigo-300/50" : "text-indigo-400"}`}>
                        {isGenerating ? (
                ...
                <button type="button" onClick={onClose} className={`grid h-7 w-7 shrink-0 place-items-center rounded-full text-xs transition ${closeBtnCls}`}>
                    ✕
                </button>
            </div>

            {/* ── Messages area ── */}
            <div className={`flex flex-1 flex-col gap-2 overflow-y-auto px-3 py-4 ${msgAreaBg}`}>
                {messages.length === 0 ? (
                    /* Empty state */
                    <div className="flex flex-col gap-3">
                        ...
                        <div className="flex items-end gap-2">
                            ...
                            <div className={`max-w-[88%] rounded-2xl rounded-bl-sm px-3.5 py-2.5 text-[13px] leading-relaxed ${isDark ? "bg-[#1e1e30] text-white/80" : "bg-white text-slate-700 shadow-sm border border-slate-100"}`}>
                                Hi! Describe any diagram and I&apos;ll draw it on your canvas. Try an example below:
                            </div>
                        </div>
                        ...
                        <p className={`ml-8 mt-1 text-[11px] ${tipText}`}>
                            You can also say &quot;improve colors&quot; or &quot;add a caching layer&quot; to modify existing shapes.
                        </p>
                    </div>
                ) : (
                    messages.map((msg, i) => {
                        ...
                        return (
                            <div key={msg.id} className={`flex items-end gap-2 ${isUser ? "flex-row-reverse" : "flex-row"} ${prevSameRole ? "mt-0.5" : "mt-2"}`}>
                                {/* Avatar — only show on first of a group */}
                                {!isUser && (
                                    <div className={`flex h-6 w-6 shrink-0 items-center justify-center rounded-full text-[10px] text-white ${prevSameRole ? "invisible" : ""}`}
                                        style={{background: "linear-gradient(135deg,#6366f1,#8b5cf6)"}}>
                                        ✦
                                    </div>
                                )}
                                ...
                                <div className="flex max-w-[82%] flex-col gap-0.5">
                                    ...
                                    >
                                        {msg.isGenerating ? (
                                            ...
                                            <span>
                                                {!isUser && !msg.isError && msg.shapeCount !== undefined && "✓ "}
                                                {!isUser && msg.isError && "⚠ "}
                                                {msg.text}
                                            </span>
                                        )}
                                    ...
                                    {/* Timestamp */}
                                    <span className={`px-1 text-[10px] ${isUser ? "text-right" : "text-left"} ${subtext}`}>
                                        {msg.time}
                                    </span>
                                </div>
                            </div>
            ...
            {/* ── Input bar ── */}
            <div className={`shrink-0 border-t px-3 pb-3 pt-2.5 ${dividerBg.replace("bg-", "border-")}`}>
                <div className={`flex items-end gap-2 rounded-2xl border px-3 py-2 transition-all focus-within:ring-2 ${inputBg}`}>
                    <textarea
                        ref={textareaRef}
                        id="ai-prompt-input"
                        rows={1}
                ...
                <p className={`mt-1 text-center text-[10px] ${subtext}`}>Enter · Shift+Enter for newline</p>
            </div>
        </div>
    );
}
...
export function AiTriggerButton({onClick, isActive, isDark}: AiTriggerButtonProps) {
    return (
        <button
