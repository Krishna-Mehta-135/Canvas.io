...
export default function ResetPasswordPage() {
    ...
    const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
        ...
        try {
            ...
            const message = submitError?.response?.data?.message || "Unable to reset password.";
            setError(message);
        } finally {
    ...
    return (
        ...
        >
            ...
            >
                ...
                <p className="mt-1 text-sm opacity-75">Set a new password for your account.</p>

                {!success && (
                    <form onSubmit={handleSubmit} className="mt-5 space-y-3">
                        <div>
                            <label className="mb-1 block text-xs font-semibold opacity-75" htmlFor="reset-password">
                                New password
                            </label>
                            <input
                                id="reset-password"
                        ...
                        <div>
                            <label className="mb-1 block text-xs font-semibold opacity-75" htmlFor="reset-confirm-password">
                                Confirm password
                            </label>
                            <input
                ...
                {error && <p className={`mt-3 text-xs ${isDark ? "text-red-300" : "text-red-700"}`}>{error}</p>}

                {success && (
                    <div className={`mt-4 rounded-xl border px-3 py-2 text-xs ${isDark ? "border-emerald-400/30 bg-emerald-500/10 text-emerald-200" : "border-emerald-300 bg-emerald-50 text-emerald-700"}`}>
