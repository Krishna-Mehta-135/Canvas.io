...
function mergeById<T extends PersistedChatMessage>(
  existing: T[],
  incoming: T[],
) {
...
function isMessageResolved(
  optimisticMessage: CanvasChatMessage,
  ...
  return persistedMessages.some((persistedMessage) => {
    ...
    if (persistedMessage.sender.id !== optimisticMessage.sender.id)
      return false;
    if (
      (persistedMessage.recipient?.id ?? null) !==
      (optimisticMessage.recipient?.id ?? null)
    )
      return false;
    if (
      (persistedMessage.shapeId ?? null) !== (optimisticMessage.shapeId ?? null)
    )
      return false;
...
export function useCanvasChat({
  ...
  const [participants, setParticipants] = useState<
    RoomChatBootstrap["participants"]
  >([]);
      ...
      .then((response) => {
        ...
        setGroupMessages(
          mergeById(
            ...
            realtimeMessagesRef.current.filter(
              (message) => message.kind === "group",
            ),
          ),
        );
        setDirectMessages(
  ...
  useEffect(() => {
    ...
    const incomingGroup = realtimeChatMessages.filter(
      (message) => message.kind === "group",
    );
    const incomingDirect = realtimeChatMessages.filter(
      (message) => message.kind === "direct",
    );
    const incomingComments = realtimeChatMessages.filter(
      (message) => message.kind === "comment",
    );
  ...
  const onlineUserIds = useMemo(
    () =>
      new Set(
        (presenceState.presences ?? []).map((presence) => presence.userId),
      ),
  ...
  const mergedParticipants = useMemo(() => {
    ...
    if (currentUserId && !next.has(currentUserId)) {
      const selfPresence = (presenceState.presences ?? []).find(
  ...
  const getDirectMessagesForUser = useCallback(
    (partnerUserId: string) =>
      [
        ...directMessages,
        ...optimisticMessages.filter((message) => message.kind === "direct"),
      ].filter((message) => {
  ...
  const createOptimisticMessage = useCallback(
    (
      kind: PersistedChatMessage["kind"],
      body: string,
      options?: { recipient?: ChatParticipant | null; shapeId?: string | null },
    ) => {
      const selfParticipant =
        mergedParticipants.find(
          (participant) => participant.id === currentUserId,
        ) ??
        ((presenceState.presences ?? []).find(
          (presence) => presence.userId === currentUserId,
        )
          ? {
              id: currentUserId!,
              name: (presenceState.presences ?? []).find(
                (presence) => presence.userId === currentUserId,
              )!.userName,
              handle: null,
              photo: null,
  ...
  const sendDirectMessage = useCallback(
    (recipientUserId: string, body: string) => {
      ...
      const recipient =
        mergedParticipants.find(
          (participant) => participant.id === recipientUserId,
        ) ?? null;
      const optimisticMessage = createOptimisticMessage("direct", body, {
        recipient,
      });
      if (!optimisticMessage) {
        return false;
  ...
  const sendComment = useCallback(
    (shapeId: string, body: string) => {
      ...
      const optimisticMessage = createOptimisticMessage("comment", body, {
        shapeId,
      });
      if (!optimisticMessage) {
        return false;
  ...
  const displayedGroupMessages = useMemo(
    () =>
      mergeById(
        groupMessages,
        optimisticMessages.filter((message) => message.kind === "group"),
      ),
