...
*   **UX Latency (Polling):** The user has to wait for the next poll cycle (e.g., every 2 seconds) to see their result.
    *   *Trade-off:* We accepted this for **Architecture Simplicity**. We don't need to track which worker is talking to which user; the database acts as the mailbox.
*   **JSON Enforcement:** Gemini sometimes deviates from the schema.
    *   *Trade-off:* We implemented a robust "Repair & Retry" loop in the worker. This adds latency but ensures the frontend never crashes due to malformed shape data.
