...
*   **Payload Size vs. Simplicity:** We send full snapshots rather than deltas (CRDTs). 
    *   *Trade-off:* This uses more bandwidth but eliminates the need for complex merge-conflict logic. We chose this for **Phase 1 Reliability**.
*   **Memory vs. Latency:** WS nodes cache room state in memory.
    *   *Trade-off:* This consumes more RAM on the server but allows us to serve "Join Room" requests in <5ms without hitting the primary database.
