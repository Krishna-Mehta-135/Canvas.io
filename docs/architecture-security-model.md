...
*   **Optimistic Concurrency Control:** The Redis-backed authority checks that the incoming version is *exactly* `current_version`.
...
*   **Performance vs. Strictness:** Checking membership on every WebSocket message would be too slow.
    *   *Trade-off:* We check membership once during the `join_room` handshake. If a user's access is revoked while they are in the room, we rely on a "Revocation Broadcast" to disconnect them.
*   **Token Revocation Latency:** Standard JWTs cannot be revoked until they expire.
    *   *Trade-off:* By including the `tokenVersion` in the JWT payload and checking it against the database (or a fast Redis cache), we achieve near-instant revocation at the cost of one DB/Redis lookup.
