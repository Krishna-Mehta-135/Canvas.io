...
export function getHandleAtPoint(
  ...
  if (shape.type === "line" || shape.type === "arrow") {
    if (
      Math.abs(x - shape.x1) <= CONNECTOR_HANDLE_SIZE &&
      Math.abs(y - shape.y1) <= CONNECTOR_HANDLE_SIZE
    ) {
      return "start";
    }

    if (
      Math.abs(x - shape.x2) <= CONNECTOR_HANDLE_SIZE &&
      Math.abs(y - shape.y2) <= CONNECTOR_HANDLE_SIZE
    ) {
      return "end";
    }
  }
