...
export function createTextEditingController(
  options: CreateTextEditingControllerOptions,
) {
  const {
    canvas,
    ctx,
  ...
  const startTextEditing = (
    ...
    const textStartX = parentBox
      ? Math.min(
          Math.max(x, parentBox.x1 + parentPadding),
          Math.max(
            parentBox.x1 + parentPadding,
            parentBox.x2 - parentPadding - 8,
          ),
        )
      : x;
    const textStartY = parentBox
      ? Math.min(
          ...
          Math.max(
            parentBox.y1 + parentPadding,
            parentBox.y2 - parentPadding - initialLineHeight,
          ),
        )
      : y;
    ...
    const editorPoint = worldToScreenPoint(
    ...
    activeTextEditorCleanup = createInlineTextEditor({
      ...
      onCommit: ({ text, width, height, fontSize: newFontSize }) => {
        ...
        const contentHeight = Math.max(
          wrappedLineHeight,
          wrappedLines.length * wrappedLineHeight,
        );
